  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        const monthlyStart = currentMonth.toISOString().split('T')[0];
        const monthlyTasks = data.filter((task) => task.due_date === monthlyStart);


        setTasks({
          todo: monthlyTasks.filter((task) => task.status === 'pending'),
          inProcess: monthlyTasks.filter((task) => task.status === 'inProcess'),
          done: monthlyTasks.filter((task) => task.status === 'completed'),
        });
      } catch (error) {
